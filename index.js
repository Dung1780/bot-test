...
function startBot(message) {
    ...
    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "tkien.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
	console.clear();
	logger(`tkien`, '[ DATABASE ]');
...
async function get2fa() {
	try {
		const response = await axios.get(`https://api.code.pro.vn/2fa/v1/get-code?secretKey=${config.OTPKEY}`);
